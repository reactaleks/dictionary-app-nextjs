        ...
        <div className=" mx-auto ">
          ...
          <div className="my-4 md:my-8">
            ...
            <ul className=" list-disc list-inside">
              {outputData?.meanings?.[0]?.definitions.slice(0, 5).map((item:{antonyms: [], definition:string, synonyms:[], example?: string}, index:string) => (
                <li
                  key={index}
            ...
            <div className="flex justify-between my-4 md:my-8 h-auto items-center">
              ...
              <div className="text-main_purple md:text-headings font-bold w-full flex justify-around">
                {outputData?.meanings?.[0]?.synonyms?.slice(0, 4).map((syn:string,index:string) => <div key={index}>{syn}</div>)}
              </div>
            </div>
          </div>
          ...
          <div className={`my-4 md:my-8 ${outputData?.meanings?.[1]?.definitions == undefined ? 'hidden' : null}`}>
            ...
            <ul className=" list-disc list-inside ">
              {outputData?.meanings?.[1]?.definitions.slice(0,5).map((item:{antonyms: [], definition:string, synonyms:[], example?: string}, index:string) => (
                ...
                >
                  ...
                  <div className="">{item?.example != undefined ? '&quot;' + item?.example + '&quot;' : null}  {item?.example}</div>
                </li>
              ))}
            </ul>
          </div>
